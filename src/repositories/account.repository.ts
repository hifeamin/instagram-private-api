...
import { AccountOnboardingOptions } from 'src/types/account.onboarding.options';
...
export class AccountRepository extends Repository {
  ...
  public async fetchHeaders(challengeType: string = 'signup') {
    const { body } = await this.client.request.send({
  ...
  public async dynamicOnboarding(options: AccountOnboardingOptions) {
    const { body } = await this.client.request.send({
      ...
      form: this.client.request.sign({
        ...
        progress_state: options.progress_state ?? 'finish',
        phone_id: this.client.state.phoneId,
        fb_installed: options.fb_installed ?? false,
        locale: this.client.state.language,
        timezone_offset: this.client.state.timezoneOffset,
        network_type: this.client.state.radioType,
        guid: this.client.state.uuid,
        is_ci: options.is_ci ?? false,
        android_id: this.client.state.deviceId,
        waterfall_id: this.client.state.uuid,
        reg_flow_taken: options.reg_flow_taken ?? 'phone',
  ...
  public async newAccountNuxSeen() {
    const { body } = await this.client.request.send({
      ...
      form: {
        ...
        // tslint:disable-next-line:max-line-length
        device_token:
          'dYJKzd30nm8%3AAPA91bE1kDVJJg_YhVxPCgho3Z2Wtq64_kTm2Qo4mnmwg6UQHeYYRzp6XZ6Yo199S2nOTAccCxlVbBw5cKD0PjwMQ_kwlDWMQ-owdXVmKmi0DaR7BrlIQ2GzCZ6E3tkJiqC-GETRsfeZ',
        _csrftoken: this.client.state.cookieCsrfToken,
        guid: this.client.state.uuid,
        _uuid: this.client.state.uuid,
  ...
  public async banyan(viewsParam) {
    const { body } = await this.client.request.send({
      ...
      qs: {
        ...
        views:
          viewsParam ??
          '["story_share_sheet","direct_user_search_nullstate","forwarding_recipient_sheet","threads_people_picker","direct_inbox_active_now","group_stories_share_sheet","reshare_share_sheet","direct_user_search_keypressed"]',
      },
    });
    return body;
  }
